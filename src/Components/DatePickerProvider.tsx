...
const DatePickerProvider = ({
  ...
  const [innerSelectedDate, innerSetSelectedDate] = useState<Date>(
    options.defaultDate ? new Date(options.defaultDate.getTime()) : new Date(),
  );
  const selectedDate = selectedDateState?.[0] ?? innerSelectedDate;

  const setSelectedDate = selectedDateState?.[1] ?? innerSetSelectedDate;

  const [showSelectedDate, setShowSelectedDate] = useState<boolean>(
